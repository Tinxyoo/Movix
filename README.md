...
- 🎯 **专为 DeepSeek V4 打造** — 原生 `reasoning_effort`(4 档)/ `thinking_mode`(token 级解析)/ Pro ↔ Flash 双模型路由
...
**预编译二进制(推荐)** — 从 [Releases](https://github.com/Tinxyoo/Movix/releases/latest) 下载:

| 平台 | 文件 |
|------|------|
| Windows x64 | `movix-x86_64-pc-windows-msvc.zip` |
| Linux x64 | `movix-x86_64-unknown-linux-gnu.tar.gz` |
| Linux ARM64 | `movix-aarch64-unknown-linux-gnu.tar.gz` |
| macOS Intel | `movix-x86_64-apple-darwin.tar.gz` |
| macOS Apple Silicon | `movix-aarch64-apple-darwin.tar.gz` |

**一键脚本**(自动识别平台 + 校验 SHA-256):
...
**源码构建**(需 Rust 1.87+,let-chains 稳定要求):

```bash
git clone https://github.com/Tinxyoo/Movix
cd movix
cargo build --release    # 产物在 target/release/movix
```
...
movix --reasoning-effort max          # 推理强度:auto / high / max
movix tools                           # 列出可用工具
movix info                            # 查看当前配置
...
Options:
  ...
  -t, --task <TASK>               非交互模式:执行单任务
      --no-thinking               启动时禁用推理
      --reasoning-effort <EFFORT> 推理强度:auto / high / max
  -h, --help                      帮助
  -V, --version                   版本
```
...
| 混合 `thinking_mode` | token 级状态机解析 `<think>` 边界(`Scavenger` + `ArgRepair` 自愈) |
| Pro / Flash 双模型 | `Ctrl+T` 手动切换;`Selector` 按任务复杂度自动切 |
| 1M 上下文 | 分层窗口 + Prefix Cache + 语义压缩,跑大仓库不爆 |
| 384K 单次输出 | `MOVIX_MAX_TOKENS=393216`,请求自动 `min(config, 384K)` 防 400 |
| 流式 + 工具并行 | 只读工具并发,写操作串行 |
...
- 🐙 [@Tinxyoo](https://github.com/Tinxyoo)
