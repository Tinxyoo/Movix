...
pub use self::parallel_dispatch::{DispatchPlan, DispatchResult, ParallelDispatcher};
...
impl MovixAgent {
    ...
    pub fn with_system_prompt_and_tools(
        ...
        let model_selector = AdaptiveModelSelector::new();
        ...
        let snapshot_repo = if which_git() {
            let repo = SnapshotRepo::new(&config.workspace);
    ...
    /// 根据消息历史自适应选择最优模型
    pub fn auto_select_model(&mut self, messages: &[ChatMessage]) -> Option<ModelConfig> {
        let config = self.workspace.model_selector.select_for_message(messages)?;
        if config.model_type.name() != self.core.config.model {
            self.core.config.model = config.model_type.name().to_string();
            self.core.config.reasoning_effort = config.reasoning_effort.clone();
            // 修复(Medium #M8):原实现不同步分层上下文的 reasoning_effort,导致 tiered
            // 按旧 effort 分配 token 预算而 LLM 用新 effort。与手动入口 set_reasoning_level
            // 保持一致,这里也同步 tiered 配置。
            if let Some(tc) = self.ctx.tiered_mut() {
                tc.set_reasoning_effort(&config.reasoning_effort);
            }
            if let Ok(new_client) = DeepSeekClient::new(self.core.config.clone()) {
    ...
    fn prepare_mutation_snapshot(
        ...
        if let Some(reason) = snapshot_mgr.should_snapshot_for_effect(effect) {
            // 修复(G-H3):快照创建失败时,原实现只 warn 后继续执行 mutating 工具,
            // 导致"承诺可回滚"的工具不可逆地执行了。现在改为 fail-close:
            // 快照失败则拒绝执行,保护可回滚性承诺。
            if let Err(e) = snapshot_mgr.before_modification(reason, &affected_paths) {
                ...
                return Err(format!(
                    "无法创建变更快照({}),已拒绝执行 {} 以保护可回滚性。请检查 ~/.movix 磁盘空间与权限。",
                    e, tool_call.function.name
                ));
            }
        }
