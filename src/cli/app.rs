...
impl App {
    ...
    /// 停止当前运行的任务（Agent 流或同步命令）。
    /// 被 Ctrl+C 和 Esc(streaming 时)共享调用。
    pub(crate) async fn stop_running(&mut self) {
        if let Some(handle) = self.agent_state.agent_handle.take() {
            if let Some(cancel_tx) = self.agent_state.agent_cancel_tx.take() {
                if let Some(ref flag) = self.agent_state.cancel_flag_handle {
            ...
            self.agent_state.agent_handle = Some(handle);
            self.finish_stream(true, Some("task cancelled by user".to_string()));
            self.recover_agent().await;
            self.load_stats(TurnStatus::Failed).await;
        } else if self.view.streaming {
            if let Some(ref flag) = self.agent_state.cancel_flag_handle {
                flag.store(true, std::sync::atomic::Ordering::Release);
            }
            self.finish_stream(
                true,
                Some("cancellation requested; waiting for command to return".into()),
            );
        }
    }
