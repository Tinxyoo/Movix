                ...
                match key.code {
                    KeyCode::Char('c') if ctrl => {
                        if let Some(anchor) = app.input.input_select_anchor {
                            ...
                            if let Some(cancel_tx) = app.agent_state.agent_cancel_tx.take() {
                            ...
                            // 将 handle 放回，让 recover_agent 通过 take + await
                            // 统一处理 Agent 恢复（abort 后 await 返回 JoinError，
                            // cancel_tx 后 await 等待 Agent 正常退出）。
                            app.agent_state.agent_handle = Some(handle);
                            app.finish_stream(true, Some("task cancelled by user".to_string()));
                            app.recover_agent().await;
                            app.load_stats(TurnStatus::Failed).await;
                            draw_needed = true;
                            ...
                            app.finish_stream(
                                true,
                                Some(
                                    "cancellation requested; waiting for command to return".into(),
                                ),
                            );
                            draw_needed = true;
                        } else {
                    ...
                    KeyCode::Enter => {
                        ...
                        app.session.last_stats = None;
                        app.view.task_list.clear();
                        app.view.thinking_expanded = false;
                        app.view.scroll_offset = 0;
                        app.view.auto_scroll = true;
                        app.view.streaming = true;
