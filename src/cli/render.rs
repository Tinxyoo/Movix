...
pub(crate) fn draw_header(f: &mut Frame, area: Rect, app: &App) {
    // 修复:顶部区域不再设底色(原 Rgb(10,12,18) 太暗,抢了 chat 区的视觉中心),
    // 改用 Color::Reset 让其透出终端默认底色,头部信息只保留前景色(品牌字 + 状态点 + 进度条)。
    ...
    let brand = Line::from(vec![
        Span::styled(
            " Movix ",
            Style::default().fg(FOCUS_CHAT).add_modifier(Modifier::BOLD),
        ),
        Span::styled(
            status_glyph,
            Style::default()
...
pub(crate) fn draw_thinking_block(lines: &mut Vec<Line>, w: usize, app: &App) {
    ...
    if app.view.thinking_expanded || app.view.streaming {
...
pub(crate) fn draw_input(f: &mut Frame, area: Rect, app: &App) {
    ...
    let border_color = if focused { FOCUS_INPUT } else { UNFOCUS_BORDER };
...
pub(crate) fn draw_stats_bar(f: &mut Frame, area: Rect, app: &App) {
    ...
    if !stats_str.is_empty() {
        // 修复:底部信息统计区域不再设底色(原 Rgb(10,12,18) 跟头部同色但跟终端对比较暗),
        // 改用 Color::Reset 让其透出终端默认底色,只保留前景文字(stats 标题行)。
...
pub(crate) fn draw_sidebar(f: &mut Frame, area: Rect, app: &App) {
    ...
    let block = Block::default()
        ...
        .title_style(
            Style::default()
                .fg(if focused { FOCUS_SIDEBAR } else { TEXT_MUTED })
                .add_modifier(Modifier::BOLD),
        );
    let inner = block.inner(area);
    f.render_widget(block, area);
    ...
    if !app.session.turn_stats.is_empty() {
        ...
        for (i, ts) in turns_slice.into_iter().enumerate() {
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(format!("T{} ", tokens_str), Style::default().fg(TEXT_MUTED)),
