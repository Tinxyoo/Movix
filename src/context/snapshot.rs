...
impl SnapshotRepo {
    pub fn new(workspace: &Path) -> Self {
        ...
        // 修复(m5):原用 FNV-1a 64 位哈希派生快照目录名,短路径理论上存在碰撞风险
        // (不同工作区哈希到同一目录 → 跨项目快照污染)。改为直接用工作区路径的
        // 规范化字符串作 key:非 [a-zA-Z0-9.-] 字符替换为 '_',前缀 'ws_'。这样目录名
        // 可读且碰撞概率远低于哈希(除非两个路径规范化后字符级相同,即同一工作区)。
        // 超长路径(>200 字符)截断后追加 FNV-1a hex 后缀防截断碰撞。
        let workspace_key = sanitize_workspace_path(workspace);
        let git_dir = home
            .join(".movix")
            .join("snapshots")
            .join(&workspace_key)
    ...
    fn ensure_secret_exclusions(&self) -> std::io::Result<()> {
        ...
        // 修复(m6):扩展密钥/凭证文件模式,覆盖 direnv / k8s / 云厂商 / 密码库 / GPG /
        // API token 等常见密钥文件,防止 git add -A 把它们提交进快照仓库明文持久化。
...
///
/// 修复(启动慢):原实现遍历整个工作区,不读 .gitignore,连 .git/node_modules/
/// target 这类动辄数万文件的目录也全扫,导致 `workspace_size_ok()` 在大仓库
/// 耗时数秒~数十秒,直接拖慢启动。此处跳过已知的"重型依赖/构建产物"目录,
/// 与 `CollaborationWatcher::ignore_patterns` 保持一致。这些目录通常也不应
/// 计入"用户工作区"体积(它们是可再生的依赖产物,不是项目源码)。
fn walk_dir_size(path: &Path) -> u64 {
    // 目录名匹配(不分大小写)即跳过。.git 必跳——其内部对象库可能极大。
    const SKIP_DIRS: &[&str] = &[
        ".git",
        "node_modules",
        "target",
    ...
    let mut total: u64 = 0;
    fn walk(path: &Path, total: &mut u64, skip_dirs: &[&str]) {
    ...
    walk(path, &mut total, SKIP_DIRS);
    total
}

/// 修复(m5):工作区路径 → 快照目录名。直接用路径的规范化字符串(非
/// [a-zA-Z0-9.-] 字符替换为 '_',前缀 'ws_')作 key,可读且碰撞概率远低于哈希。
/// 超长路径(>200 字符)截断后追加 FNV-1a hex 后缀防截断碰撞。
...
/// FNV-1a 64 位,仅用作超长路径截断时的碰撞兜底后缀(非主 key)。
...
mod tests {
    ...
    fn test_sanitize_workspace_path() {
        let h1 = sanitize_workspace_path(Path::new("/path/to/workspace"));
        let h2 = sanitize_workspace_path(Path::new("/path/to/other"));
        assert_ne!(h1, h2);
        // 前缀 + 路径分隔符被替换
        assert!(h1.starts_with("ws_"));
        assert!(!h1.contains('/'));
    }
